...
`hv` reads your LLM traffic locally, verifies task success with programmatic
...
```bash
pip install -e .
hv record events.jsonl     # ingest a trace source (metadata extracted locally)
hv report                  # ranked HTML leaderboard: CPAT, waste, success rates
```
...
output-contract extraction, golden sets, and `hv pack` — the handover pack that
