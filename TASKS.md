...
## P4 — השופט (המודל ששופט מודלים)

> החלטה 2026-09-12: שכבת "שופט" מרכזית במירדה, לא מוצר נפרד. מה שמתחת למודל הבסיס יהפוך לחינמי; מה שמחזיק זה מה שנצבר מעל — היסטוריית מדידה, נייטרליות, פנקס אמינות, ועדכון חודשי על דפוסי כשל של מודלים חדשים. מגבלה ידועה: שופט 8B לא יודע עובדות שמודל 400B יודע → מתחילים מ-faithfulness ומרחיבים ל-factuality רק עם שכבת חיפוש.
...
> `bench/hallucinate.py`: הזרקה מבוקרת על הסוללה (החלפת מספר/תאריך/ישות, טענה שלא במקור) → סט אימון עם אמת ידועה מהבנייה + סט הערכה קפוא (כמה מאות לכל ממד). אמת ידועה = `verify/` (ExitCode/JsonSchema/Regex/Equivalence).

### T23 · אימון השופט `[BUILD, GPU]`
> Qwen 3 8B + 1.7B: SFT (קלט → Verdict JSON עם נימוק מזוקק מהמושבעים) ואז RL עם תגמול מאומת (הסכמה עם האמת הידועה + קנס Brier על כיול). השוואה מול המושבעים, GPT-4o-as-judge, Lynx, Selene על RAGTruth / HaluEval / LLM-AggreFact / RewardBench. GPU שכור, 3–8K$ — עצירת החלטה לפני.

### T24 · השופט בייצור `[BUILD]`
> השופט מחליף את המושבעים, רץ in-tenant (קונטיינר) — הדרך היחידה לשפוט תוכן אמיתי בלי להפר את גבול הדייר; מתאמן מחדש כל חודש על פלטי המודלים החדשים מהדירוג השעתי; "פנקס האמינות" של מודלים = מוצר בפני עצמו. פותח את ה-Handshake לכל עומס עבודה (הפיילוט נחסם עם 75.8% unknown).

### קבלת P4
> על סט הזהב: השופט שלנו ≥ המושבעים בהסכמה עם האמת הידועה, Brier נמוך יותר, ו-≤10% low_agreement על עומס לקוח אמיתי.
